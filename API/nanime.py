...
class gogoanime():
    ...
    def cari_kuronime(query):
        try:
            url1 = f"https://45.12.2.26/?s={query}"
            session = HTMLSession()
            response = session.get(url1)
            response_html = response.text
            soup = BeautifulSoup(response_html, 'html.parser')
    ...
    #Cari Kuramanime    
    def cari_kuramanime(query):
        try:
            url1 = f"https://kuramanime.pro/anime?search={query}"
            session = HTMLSession()
            response = session.get(url1)
            response_html = response.text
            soup = BeautifulSoup(response_html, 'html.parser')
...
#                 genres.append(link.get('title'))
    ...
    def a_list():
        try:
            url = 'https://t.me/s/panimeid?q=daftar+isi%20[%20A%20]'
            session = HTMLSession()
            response = session.get(url)
            response_html = response.text
    ...
    def a_list1():
        try:
            url = 'https://t.me/s/panimeid?q=daftar%20anime%20a%20-%201'
            session = HTMLSession()
            response = session.get(url)
            response_html = response.text
    ...
    def anime_episode(animeid):
        try:
            url1 = 'https://45.12.2.26/anime/{}'.format(animeid)
            session = HTMLSession()
            response = session.get(url1)
            response_html = response.text
