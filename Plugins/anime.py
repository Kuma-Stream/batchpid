...
class Anime():
    ...
    async def event_handler_anime(event):
        ...
        (names, ids, epnums) = format.format_home_results(moenime)
        buttonss = [[], [Button.inline(
                    "Tutup", data=f"close_data")]]
        for i in range(len(names)):
            try:
    ...
    #mencari meownime
    @bot.on(events.NewMessage(pattern=r"^/kuramanime|^/kuramanime@cipdbot"))
    async def event_handler_anime(event):
        kuramanime = gogo.kuramanime()
        (names, ids, epnums) = format.format_home_results(kuramanime)
        buttonss = [[], [Button.inline(
                    "Tutup", data=f"close_data")]]
        for i in range(len(names)):
            try:
                ...
                pass
        if '/kuramanime' == event.raw_text:
            await bot.send_message(
                event.chat_id,
                'Berikut Daftar Anime Terbaru Dari Sumber kuramanime:\nUntuk Mencari Anime Silahkan Gunakan Perintah `/kuramanime judul anime`\n\n',
                 buttons=buttonss,
            )
        elif '/kuramanime' in event.raw_text:
            text = event.raw_text.split()
            text.pop(0)
            anime_name = " ".join(text)
            cari_kuramanime = gogo.cari_kuramanime(anime_name)
            try:
                (names, ids) = format.format_search_results(cari_kuramanime)
                buttons1 = [[], [Button.inline(
                    "Tutup", data=f"close_data")]]
                for i in range(len(names)):
                    if len(names[i]) > 55:
